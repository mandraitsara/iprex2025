 ...
 */
$siteName 				= 'DUMP';  					// Nom de l'archive qui sera créée	
$dirToSave 				= ROOT; 					// ROOT pour sauver tout le site
$dirsNotToSaveArray 	= array(); 	 				// Liste des répertoires à ne pas sauvegarder
$mysqlSaveDir 			= ROOT.'/../saves'; ;  		// Dossier de sauvegarde pour les fichiers de tailles de la BDD
 
$zipSaveDir = ROOT.'/../saves'; 
/** 
...
<html>
	...
	<body>
		...
		if(!empty($mysql_bdd) && !$no_sql) {
			...
			$cmd = "mysqldump --host=".$mysql_server." --user=".$mysql_login." --password='".$mysql_passwd."' ".((isset($mysql_port) && !empty($mysql_port))?"--port=".$mysql_port." ":"").$mysql_bdd." | gzip > ".$mysqlSaveDir."/DUMP-".date('Ymd')."-".date('Hi').".sql.gz";
		...
		if (!$no_zip) { // Si pas mode debug
			...
			if (!empty($mysql_bdd)) {
				...
				//$zipBdd = $mysqlSaveDir."/".$mysql_bdd.".sql.gz", $siteName.'/'.$mysql_bdd.".sql.gz";
				...
				$file_bdd_size = fopen($MysqlSaveDirStats.'/bdd_size_'.$siteName.'.txt', 'w+');
				fputs($file_bdd_size, filesize($MysqlSaveDirStats."/".$mysql_bdd.".sql.gz"));
			...
			if (file_exists($zipfile)) {
				...
				$sujet 		= "Backup BDD $siteName réussi";
				$message 	= "Backup BDD $siteName réussi le ".date('d/m/Y à H:i:s');
			...
			// Sinon (erreur)	
				...
				$sujet 		= "Backup BDD $siteName échoué";
				$message 	= "Backup BDD $siteName échoué le ".date('d/m/Y à H:i:s');
			}
