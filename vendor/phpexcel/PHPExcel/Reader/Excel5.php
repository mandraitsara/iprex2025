		...
		foreach ($this->_definedname as $definedName) {
			if ($definedName['isBuiltInName']) {
				...
				case pack('C', 0x06):
					...
					$ranges = explode(',', $definedName['formula']); // FIXM3: what if sheetname contains comma?
					...
					foreach ($ranges as $range) {
						...
						$explodes = explode('!', $range);	// FIXM3: what if sheetname contains exclamation mark?
				...
				if (count($explodes) == 2) {
					...
					//	Named Value
					//	TOD0 Provide support for named values
				}
			}
		}
        $this->_data = null;
...
//		echo '<br />';
	...
	private function _readLabel()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell value
			// tod0: what if string is very long? continue record
			if ($this->_version == self::XLS_BIFF8) {
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		//FIXM3: set $firstVisibleRow and $firstVisibleColumn
	...
	private function _readSheetLayout()
	{
		...
		if (!$this->_readDataOnly) {
			...
			switch ($sz) {
				...
				case 0x28:
					// TOD0: Investigate structure for .xls SHEETLAYOUT record as saved by MS Office Excel 2007
					return;
					break;
			}
		}
...
//			var_dump($bcBitCount);
	...
	{
		// tod0: if cell range is just a single cell, should this funciton
		...
		if (isset($this->_ref[$index])) {
			...
			switch ($type) {
				case 'internal':
					...
					// tod0: check if we have identified the whole set of special characters
