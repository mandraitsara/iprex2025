...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	...
	private function _writeCFRule(PHPExcel_Style_Conditional $conditional){
		...
		if($bFormatBorder == 1){
			...
			//@tod0 _writeCFRule() => $blockLineStyle => Index Color for left line
			//@tod0 _writeCFRule() => $blockLineStyle => Index Color for right line
			//@tod0 _writeCFRule() => $blockLineStyle => Top-left to bottom-right on/off
			//@tod0 _writeCFRule() => $blockLineStyle => Bottom-left to top-right on/off
			$blockColor = 0;
			//@tod0 _writeCFRule() => $blockColor => Index Color for top line
			//@tod0 _writeCFRule() => $blockColor => Index Color for bottom line
			//@tod0 _writeCFRule() => $blockColor => Index Color for diagonal line
