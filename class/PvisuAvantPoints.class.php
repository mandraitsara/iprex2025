<?php
/*
   _|_|_|  _|_|_|    _|
 _|        _|    _|  _|        CBO FrameWork
 _|        _|_|_|    _|        (c) 2018 Cédric Bouillon
 _|        _|    _|  _|
   _|_|_|  _|_|_|    _|_|_|_|
--------------------------------------------------------
Objet PvisuAvantPoints
Généré par CBO FrameWork le 03/09/2020 à 11:13:53
------------------------------------------------------*/
class PvisuAvantPoints {

	protected    $id,
		$id_pvisu_avant,
		$id_point_controle,
		$etat,
		$id_user,
		$date;

	public       $attributs = array();

	public function __construct(array $donnees)	{
		$this->hydrate($donnees);
	}

	public function hydrate(array $donnees)	{
		foreach ($donnees as $key => $value) {
			$method = 'set'.ucfirst(strtolower($key));
			if (method_exists($this,$method)) {
				$this->$method($value);
			}
		}
		$this->attributs = array();
	}

	/* ----------------- GETTERS ----------------- */

	public function getId() {
		return $this->id;
	}

	public function getId_pvisu_avant() {
		return $this->id_pvisu_avant;
	}

	public function getId_point_controle() {
		return $this->id_point_controle;
	}

	public function getEtat() {
		return $this->etat;
	}

	public function getId_user() {
		return $this->id_user;
	}

	public function getDate() {
		return $this->date;
	}

	/* ----------------- SETTERS ----------------- */

	public function setId($id) {
		$this->id = (int)$id;
	}

	public function setId_pvisu_avant($id_pvisu_avant) {
		$this->id_pvisu_avant = (int)$id_pvisu_avant;
		Outils::setAttributs('id_pvisu_avant',$this);
	}

	public function setId_point_controle($id_point_controle) {
		$this->id_point_controle = (int)$id_point_controle;
		Outils::setAttributs('id_point_controle',$this);
	}

	public function setEtat($etat) {
		$this->etat = (int)$etat;
		Outils::setAttributs('etat',$this);
	}

	public function setId_user($id_user) {
		$this->id_user = (int)$id_user;
		Outils::setAttributs('id_user',$this);
	}

	public function setDate($date) {
		$this->date = (string)$date;
		Outils::setAttributs('date',$this);
	}

} // FIN classe