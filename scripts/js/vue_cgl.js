...
$('document').ready(function() {
    ...
    // Mise en attente produit (modale)
    ...
    $.fn.ajax({
        ...
        return_id: 'etape'+numeroEtape,
            ...
            $.fn.ajax({
                ...
                arguments: 'mode=consoleDev&etape=' + numeroEtape + '&id=' + identifiant + '&id_froid=' + id_froid + '&vue=cgl',
...
function chargeTicketLot(etape, identifiant) {
    ...
    // Si on viens de lancer une congélation, il ne faut pas passer l'ID froid, afin de ne pas conserver la CGL en cours dans la nouvelle CGL
...
function listenerEtape0(identifiant) {
    ...
    // Nouvelle CGL
    $('.btnNouvelleCgl').off("click.btnnouvellecgl").on("click.btnnouvellecgl", function(e) {
    ...
    // CGLs en cours
    $('.btnCglsEnCours').off("click.btncglsencours").on("click.btncglsencours", function(e) {
        e.preventDefault();
        chargeEtape(101,0);
    });
...
function listenerEtape2(identifiant) {
    ...
    $(document).on('click','.carte-pdt-pagination',function(){
        ...
        $.fn.ajax({
            ...
            'script_execute':'fct_vue_cgl.php'+$(this).attr('data-url'),
            'return_id':'etape2',
            'done': function () {
                listeProduitsListener();
            }
...
function listenerEtape3(identifiant) {
    ...
    // Si on est en mode update depuis la liste des produits d'une CGL en cours, on masque le bouton "Retour" du ticket
    if (modeUpd) { $('.btnRetourCglsEnCours').hide(); }
    ...
    $('input[name=quantite]').focus(function()  { champ = 'quantite'; });
    ...
    $('.clavier .btn').off("click.appuietoucheclaviernbcolis").on("click.appuietoucheclaviernbcolis", function(e) {
        ...
        if (modeUpd && $('.methode-maj').prop('checked')) {
            ...
            var info          = "Total produit mis à jour : " + nb_colis + ' colis, ' + poids_pdt + ' kg.';
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 4 (Température de début de congélation)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 5 (Départ congélation)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function listenerEtape5(identifiant) {
    ...
    // Début CGL
    $('.btnDebutCgl').off("click.btndebutcgl").on("click.btndebutcgl", function(e) {

        e.preventDefault();

        var id_froid = parseInt($('#inputIdFroid').val());

        // Enregistrement du début de cgl
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute':'fct_vue_cgl.php',
            'arguments': 'mode=saveDebutCgl&id_froid=' + id_froid,
            'callBack': function (retour) {
                if (parseInt(retour) < 0) { alert("Une erreur est survenue !\r\nEnregistrement impossible...\r\nCode erreur : JKTZSWKP"); return false; }
    ...
    }); // FIN bouton début CGL

} // FIN listener


// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 6 (Température de fin de congélation)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 7 (Bloquage suite alerte temp. de fin cgl)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
function listenerEtape9(identifiant) {
    ...
    $('.btnConformiteCgl').off("click.btnconformitecgl'").on("click.btnconformitecgl", function(e) {
        e.preventDefault();

        var conformite  = parseInt($(this).data('conformite'));
        var id_froid    = parseInt($('#inputIdFroid').val());

        if (conformite < 0 || conformite > 1 || isNaN(conformite) || conformite === undefined) { alert("Une erreur est survenue !\r\nCode erreur : E5EKYIH9"); return false; }

        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute': 'fct_vue_cgl.php',
            'arguments': 'mode=conformiteCgl&id_froid='+id_froid + '&conformite='+conformite,
            'callBack': function(retour) {

                retour+= ''; // Debug mauvaise interprétation de retours
                if (parseInt(retour) < 0) { alert('Une erreur est survenue !\r\nCode erreur : VRKEXAIX'); return false; }
...
function listenerEtape10(identifiant) {
    ...
    $('.togglemaster').change(function () {
        ...
        if (id_lot_pdt_froid < 1 || isNaN(id_lot_pdt_froid)) {
            alert("ERREUR !\r\nIdentification du produit froid impossible.\r\nCode erreur : 6SBXG4M5");
            return false;
        }
    ...
    $('.btnChangePoidsPdt').off("click.btnchangepoidspdt'").on("click.btnchangepoidspdt", function(e) {
        ...
        var id_lot_pdt_froid = parseInt($(this).data('id-lot-pdt-froid'));
        var identifiant      = id_lot_pdt_froid; // On intègre une variable pour adapter le comportement du callback Valider
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 11 (Reprise produit en attente)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function listenerEtape11(identifiant) {
    ...
    if (vrac === 1) {
        $('input[name=nb_colis]').val(1);
        $('input[name=poids_pdt]').addClass('champ-actif');
        $('input[name=nb_colis]').removeClass('champ-actif');
        $('input[name=poids_pdt]').focus();
    } else {
        $('input[name=nb_colis]').focus();
    }
    ...
    $('.btnRetourProduits').off("click.btnretourproduits'").on("click.btnretourproduits", function(e) {
        ...
        // On vient forcément du catalogue...
        chargeEtape(1,0);
    ...
    $('.carte-palette').off("click.cartepalette'").on("click.cartepalette", function(e) {
        ...
        // Si on a pas rajouté de poids, on stope, sinon ça effectue des changement de compos arbitraires sur l'ensemble qui finissent par pourrir la base !
        if (isNaN(poids)) {

            console.log(poids);


            $('#modalInfoBody').html("<div class='alert alert-danger'><i class='fa fa-exclamation-circle fa-lg mb-2'></i><p>Renseignez le nombre de colis<br>avant de sélectionner une palette... </p></div>");
            $('#modalInfo').modal('show');
            return false;
        }
        ...
        if (palette_restant_poids >= poids) {
            ...
            // On rajoute le flag permettant de ne pas créer de compo à l'enregistrement
            $('#skipCreateCompoSave').val(1);
    ...
    $('.btnModalConfirmOk').off("click.btnModalConfirmOk").on("click.btnModalConfirmOk", function(e) {
        ...
        // ----------------------------------------
        // (Listener Etape 11 -> Modal confirm OK)
        // Sélection palette en surcapacité
        // ----------------------------------------
        if (data_confirm_contexte === 'paletteOverCapacite') {
            ...
            if ($('#forceSaveProduit').val() !== '') {
                var saveProduitParams = $('#forceSaveProduit').val().split(',');
                if (!saveProduitParams[2].length) { return false; }
                saveProduitAttente(saveProduitParams[0], saveProduitParams[1], saveProduitParams[2]);
            }
    ...
    // ----------------------------------------
    // (Listener Etape 11)
    // Définition du champ au focus
    // ----------------------------------------
    $('.inputNbColis, .inputPoidsPdt').focus(function() {
        $('.inputNbColis, .inputPoidsPdt').removeClass('champ-actif');
        $(this).addClass('champ-actif');
    });
    ...
    $('.clavier .btn').off("click.appuietoucheclaviernbcolis").on("click.appuietoucheclaviernbcolis", function(e) {
        ...
        if (touche === 'V') {
            ...
            // Contrôle de cohérence du poids et du nombre de colis
            if (nbColis < 1 || isNaN(nbColis) || poidsPdt < 0.0 || isNaN(poidsPdt)) {
            ...
            // On véfifie qu'on ne dépasse pas le max mis en attente
            var nb_colis_old = parseInt($('input[name=nb_colis_old]').val());
            var poids_pdt_old = parseFloat($('input[name=poids_pdt_old]').val());

            if (vrac === 1) {
                nb_colis_old = nbColis;
            }

            if (nbColis > nb_colis_old || poidsPdt > poids_pdt_old) {
                $('#modalInfoBody').html("<div class='alert alert-danger'><i class='fa fa-exclamation-circle fa-lg mb-2'></i><p>Il n'y a que "+poids_pdt_old+" kg en attente pour ce produit...</p></div>");
                $('#modalInfo').modal('show');
                return false;
            }
            ...
            // On vérifie qu'une palette est bien sélectionnée
            if ((isNaN(id_compo) || id_compo === undefined || id_compo === 0 || isNaN(id_palette) || id_palette === undefined || id_palette === 0) || !palette_selectionne) {
            ...
            // Si on a pas cliqué sur une palette il faut créer une nouvelle compo pour ces quantités, sans effacer l'ancienne.
            var skip_add_palette = parseInt($('#skipCreateCompoSave').val()) > 0;

            // Si on a pas à ignorer la création de la composition...

            // ----------------------------------------
            // (Listener Etape 11 -> Clavier -> Valider)
            // Création de la composition de palette
            // ----------------------------------------
            if (!skip_add_palette) {
                ...
                if (palette_restant_poids >= poids) {
                   ...
                   // SI on a pas de palette selectionnée masquée...
                    if ($('.palette-selectionnee-masquee').length === 0) {

                        // On retire la compo de référence pour ne pas qu'elle soit supprimée en BDD
                        $('#inputIdCompo').val(0);

                    } // FIN test nécéssité de RAZ la compo historique
            ...
            // Enregistrement du produit au traitement
            saveProduitAttente(identifiant, id_palette, id_compo);
            return false;
        ...
        // ----------------------------------------
        // (Listener Etape 11 -> Clavier)
        // Touche [T] - (TOUT) 100% de reprise
        // ----------------------------------------
        if (touche === 'T') {

            $('input[name=nb_colis]').val($('input[name=nb_colis_old]').val());
            $('input[name=poids_pdt]').val($('input[name=poids_pdt_old]').val());
        ...
        var champ = $('.champ-actif').attr('name');
        if (champ === undefined) { champ = 'nb_colis'; }
        ...
        if (champ === 'poids_pdt') {
            ...
            // Si le poids actuel dans le input est le max du pdt en attente, alors on efface pour saisir le nouveau
            if (poidsProduit === poids_pdt_old_val) {
                $('.inputPoidsPdt').val('');
                poidsProduit = '';
            }
            ...
            // Si le nb de colis actuel dans le input est le max du pdt en attente, alors on efface pour saisir le nouveau
            if (nbcolis === nb_colis_old_val) {
                $('.inputNbColis').val('');
                nbcolis = '';
            }
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 101 (Liste des congélations en cours)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function listenerEtape101(identifiant) {
    ...
    // Sélection d'une CGL pour cartecgl
    $('.carte-cgl').off("click.cartecgl'").on("click.btnretouretape0", function(e) {
...
function listenerEtape100(identifiant) {
    ...
    // Confirmation dans la modale de la clôture de la congélation

    $('.btnConfirmClotureCgl').off("click.btnconfirmcloturecgl'").on("click.btnconfirmcloturecgl", function(e) {

        e.preventDefault();

        var id_froid = parseInt($('#inputIdFroid').val());
        if (id_froid === 0 || isNaN(id_froid) || id_froid === undefined) { alert('Une erreur est survenue !\r\nCode erreur : ULLXSRCT');return false;}

        // On enregistre (visa, staut, etc...)
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute': 'fct_vue_cgl.php',
            'arguments': 'mode=clotureCgl&id_froid='+id_froid,
            'done': function() {
                location.reload();
            }
        }); // FIN aJax
...
function listenerTicketLot() {
    ...
    // Chargement du contenu de la modale de confirmation de fin de congélation à son ouverture
    ...
    // Retour vers la température de début de cgl
    ...
    // Retour vers les congélations en cours
    $('.btnRetourCglsEnCours').off("click.btnretourcglsencours'").on("click.btnretourcglsencours", function(e) {

        e.preventDefault();
        $('#inputIdFroid').val('');
        chargeEtape(101, 0);

    }); // FIN Retour vers les CGLs en cours

    // Bouton continuer la congéaltion
    $('.btnContinuerCgl').off("click.btncontinuercgl'").on("click.btncontinuercgl", function(e) {

        e.preventDefault();
        var etape_suivante = parseInt($(this).data('etape-suivante'));
        if (etape_suivante === 0 || etape_suivante === undefined || isNaN(etape_suivante)) { alert('Une erreur est survenue !\r\nCode erreur : GRLXQMYE'); return false; }
        chargeEtape(etape_suivante, 0);

    }); // FIN Bouton continuer la congéaltion
    ...
    // Bouton fin étiquetage -> vers température début CGL
    ...
    $('#modalNouvelEmballage').on('show.bs.modal', function (e) {
        ...
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute': 'fct_emballages.php',
            'arguments': 'mode=modalNouvelEmballageFront&vue_code=cgl',
            'return_id' : 'modalNouvelEmballageBody',
            'done': function() {
...
function modalNouvelEmballageFrontEtape2Listener() {
    ...
    $('.carte-new-emb-defaut').off("click.cartenewembdefaut'").on("click.cartenewembdefaut", function(e) {
        ...
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute': 'fct_emballages.php',
            'arguments': 'mode=setNewEmballageEnCours&id_emb='+id_emb+'&id_froid='+id_froid+'&code_vue=cgl',
            'done': function() {
...
function modalNouvelEmballageFrontListener() {
    ...
    $('.carte-fam-add-emb').off("click.cartefamaddemb'").on("click.cartefamaddemb", function(e) {
        ...
        var id_old_emb = parseInt($(this).data('id-old-emb'));
        if (id_fam === undefined || id_fam === 0 || isNaN(id_fam)) { alert("Identifaction de la famille impossible.\nCode erreur : TTUUFWRP"); return false; }
...
function saveTemperature(id_froid) {
    ...
    $.fn.ajax({
        ...
        'callBack': function (retour) {
            ...
            if (parseInt(retour) < 0) { alert("Une erreur est survenue !\r\nEnregistrement impossible...\r\nCode erreur : KENMBTJS"); return false; }

            // Si alerte sur la température, on bloque le workflow !
            if (parseInt(retour) === 42) {

                chargeEtape(7,0);
                return false;

            } // FIN test alerte température
...
function saveProduit(identifiant, id_palette, id_compo) {
    ...
    var nbColis     = parseInt($('.inputNbColis').val());
    var poidsPdt    = parseFloat($('.inputPoidsPdt').val());
    var quantite    = parseFloat($('.inputQuantite').val());
    ...
    if (modeUpd) {
        ...
        setTimeout(function(){
            ...
            $.fn.ajax({
                'rep_script_execute': "../scripts/ajax/", // Gestion de l'URL Rewriting
                'script_execute': 'fct_vue_cgl.php',
                'arguments': 'mode=updPdtLotPoids&id_lot_pdt_froid='+identifiant+'&nb_colis='+nbColis+'&poids='+poidsPdt+'&quantieme='+quant+'&poids_add='+poids_pdt_add+'&nb_colis_add='+nb_colis_add+'&id_palette='+id_palette+'&id_compo='+id_compo+'&ajout='+ajout + '&quantite='+quantite,
                'done': function () {
    ...
    // Sinon (pas mode update), c'est qu'on est dans l'ajout d'un nouveau produit dans le traitement
    } else {
        ...
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/", // Gestion de l'URL Rewriting
            'script_execute': 'fct_vue_cgl.php',
            'arguments': 'mode=addPdtFroid&id_pdt='+id_pdt+'&id_lot='+id_lot+'&id_froid='+id_froid+'&nb_colis='+nbColis+'&poids='+poidsPdt+'&quantieme='+quant+'&id_palette='+id_palette+'&id_compo='+id_compo + '&quantite='+quantite, 
            'callBack': function (id_froid) {

                // Gestion des erreurs
                if (parseInt(id_froid) < 0) {
                    alert("Une erreur est survenue !\r\nCode erreur : PKXIUKNE-"+id_pdt+"-"+id_lot+"-"+id_froid+"-"+nbColis+"-"+poidsPdt+"-"+quant+"-"+id_palette);
                    return false;
                }

                // Intégration de l'id froid si on viens de la créer et que le retour est valide
                if (parseInt(id_froid) > 0 && id_froid !== undefined && !isNaN(id_froid) && parseInt($('#inputIdFroid').val()) === 0) {
                    $('#inputIdFroid').val(id_froid);
                }
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Fonction déportée -> Reprise partielle ou complète produit en attente
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function saveProduitAttente(identifiant, id_palette, id_compo) {
    'use strict';
    ...
    if (isNaN(parseInt(identifiant)) || parseInt(identifiant) === 0 || parseInt(identifiant) === undefined ||
        isNaN(parseInt(id_palette))  || parseInt(id_palette)  === 0 || parseInt(id_palette)  === undefined ||
        isNaN(parseInt(id_compo))    || parseInt(id_compo)    === 0 || parseInt(id_compo)    === undefined ||
        isNaN(parseInt(nbColis))     || parseInt(nbColis)     === 0 || parseInt(nbColis)     === undefined ||
        isNaN(parseFloat(poidsPdt))  || parseFloat(poidsPdt)  === 0 || parseFloat(poidsPdt)  === undefined) {
        alert("ERREUR !\r\nIdentification des données impossible...\r\nCode erreur : 7NEZ1MV7-"+identifiant+"/"+id_palette+"/"+id_compo+"/"+nbColis+'/'+poidsPdt);
        return false;
    }
    ...
    $.fn.ajax({
        'rep_script_execute': "../scripts/ajax/", // Gestion de l'URL Rewriting
        'script_execute': 'fct_vue_cgl.php',
        'arguments': 'mode=reprisePdtAttente&id_lot_pdt_froid='+identifiant+'&nb_colis='+nbColis+'&poids='+poidsPdt+'&id_palette='+id_palette+'&id_compo='+id_compo+'&id_froid='+id_froid,
        'callBack': function (retour) {

            retour+= '';
            if (retour === '') { retour = '3ADHJVOQ'; }

            var id_froid = parseInt(retour);

            if (id_froid < 1 || isNaN(id_froid)) {
                alert('ERREUR !\r\nEchec de la reprise du produit...\r\nCode erreur :'+retour);
            }

            // Si on a créé une op de froid, on enregistre l'id pour le chargement de l'étape suivante...
            if (id_froid > 1) {
                $('#inputIdFroid').val(id_froid);
            }
...
function addProduitPalette(id_palette, supprime) {
    ...
    $.fn.ajax({
        'rep_script_execute': "../scripts/ajax/",
        'script_execute': 'fct_palettes.php',
        'arguments': 'mode=affecteCompoPalette&id_palette=' + id_palette + '&id_produit=' + id_produit + '&nb_colis='+nb_colis+'&poids='+poids+'&old_compo='+id_compo+'&id_client='+id_client+'&id_lot_pdt_froid='+id_lot_pdt_froid+'&supprime='+supprime + '&quantite='+quantite,
        'callBack': function (retour) {
...
function nouvellePaletteListener(identifiant) {
    ...
    $('.btnClientNewPalette').off("click.btnClientNewPalette").on("click.btnClientNewPalette", function(e) {
        ...
        if (isNaN(num_palette) || num_palette === 0) {
            alert("ERREUR !\r\nIdentification du numéro de palette impossible.\r\nCode erreur : VAGXIING");
            return false;
        } // FIN gestion des erreurs

        if (isNaN(id_client) || id_client === 0) {
            alert("ERREUR !\r\nIdentification du client impossible.\r\nCode erreur : R8U903IP");
            return false;
        } // FIN gestion des erreurs

        // Dans un premier temps, on tente de récupérer l'id_produit différement (cas de reprise de produit en attente par exemple)
        if ((id_produit === undefined || id_produit === 0 || isNaN(id_produit)) && $('#inputIdProduit').length) {
            id_produit = parseInt($('#inputIdProduit').val());
        }

        if (id_produit === undefined || id_produit === 0 || isNaN(id_produit)) {
            alert("ERREUR !\r\nIdentification du produit impossible...\r\nCode erreur : C8F7NJK4");
            return false;
        }
        if (nb_colis === undefined || nb_colis === 0 || isNaN(nb_colis) || poids === undefined || poids === 0) {

            $('#modalPaletteFront').modal('hide');
            $('#modalInfoBody').html("<div class='alert alert-danger'><i class='fa fa-exclamation-circle fa-lg mb-2'></i><p>Renseignez le nombre de colis<br>avant de créer une nouvelle palette...(4)</p></div>");
            $('#modalInfo').modal('show');
