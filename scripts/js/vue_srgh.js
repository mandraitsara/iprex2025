/**
 ------------------------------------------------------------------------
 JS - Vue Surgélation Horizontale
...
function chargeEtape(numeroEtape, identifiant) {
    ...
    $.fn.ajax({
        'rep_script_execute': "../scripts/ajax/", // Gestion de l'URL Rewriting
        'script_execute': 'fct_vue_srgh.php',
...
function listenerEtape2(identifiant) {
    ...
    $(document).on('click','.carte-pdt-pagination',function(){
        ...
        $.fn.ajax({
            ...
            'script_execute':'fct_vue_srgh.php'+$(this).attr('data-url'),
            'return_id':'etape2',
            'done': function () {
                listeProduitsListener();
            }
...
function listenerEtape5(identifiant) {
    ...
    $('.btnDebutSrg').off("click.btndebutsrg").on("click.btndebutsrg", function(e) {
        ...
        $.fn.ajax({
            ...
            'callBack': function (retour) {
                if (parseInt(retour) < 0) { alert("Une erreur est survenue !\r\nEnregistrement impossible...\r\nCode erreur : AVLKRBMR"); return false; }
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// Listener de l'étape 12 (Loma apres)
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function listenerEtape12(identifiant) {
    "use strict";

    chargeTicketLot(4, identifiant); // On prends déjà le ticket du l'étape 4 car on ne va pas garder celui de la section produits
    // Bouton test ok / ko
    $('.btn-loma').off("click.btnloma").on("click.btnloma", function(e) {
    ...
    $('.btnConformiteSrg').off("click.btnconformitesrg'").on("click.btnconformitesrg", function(e) {
        ...
        $.fn.ajax({
            ...
            'callBack': function(retour) {
                ...
                retour+= ''; // Debug mauvaise interprétation de retours
                if (parseInt(retour) < 0) { alert('Une erreur est survenue !\r\nCode erreur : BRCXWNSP'); return false; }
...
function listenerEtape11(identifiant) {
    ...
    $('.clavier .btn').off("click.appuietoucheclaviernbcolis").on("click.appuietoucheclaviernbcolis", function(e) {
        ...
        if (touche === 'V') {
            ...
            if (vrac === 1) {  nb_colis_old = nbColis;  }
...
function listenerTicketLot() {
    ...
    // Bouton fin étiquetage -> vers LOMA APRES
    ...
    $('#modalNouvelEmballage').on('show.bs.modal', function (e) {
        ...
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute': 'fct_emballages.php',
            'arguments': 'mode=modalNouvelEmballageFront&vue_code=srgh',
            'return_id' : 'modalNouvelEmballageBody',
            'done': function() {
...
function modalLomaListener(id_pdt, id_lot, id_froid) {
    "use strict";
    if (isNaN(parseInt(id_pdt)) || parseInt(id_pdt) === 0 || id_pdt === undefined) { alert("Identification du produit impossible !\r\bnCode erreur : S1L0353Q"); return false; }
    if (isNaN(parseInt(id_lot)) || parseInt(id_lot) === 0 || id_lot === undefined) { alert("Identification du lot impossible !\r\bnCode erreur : N6VA5YZO"); return false; }
    if (isNaN(parseInt(id_froid)) || parseInt(id_froid) === 0 || id_froid === undefined) { alert("Identification de la surgélation impossible !\r\nCode erreur : GUUO8ZJ2 / " + id_pdt+'-'+id_froid+'-'+id_lot); return false; }
    ...
    $('.btnCtrlLoma').off("click.btnctrlloma").on("click.btnctrlloma", function(e) {
        ...
        if (loma < 0 || loma > 1 || isNaN(loma) || loma === undefined) { alert("Identification du choix impossible !\r\bnCode erreur : QYMQI27T"); return false; }
        ...
        if (loma === 1) {
            ...
            $.fn.ajax({
                ...
                'callBack': function (retour) {
                    ...
                    // Surgélation horizontale, ici on ne retourne pas à la liste, si on a dit oui, on va à l'étape 7 en passant le produit
                    chargeEtape(7, retour); // Retour = id_lot_pdt_froid
...
function modalNouvelEmballageFrontListener() {
    ...
    $('.carte-fam-add-emb').off("click.cartefamaddemb'").on("click.cartefamaddemb", function(e) {
        ...
        var id_old_emb = parseInt($(this).data('id-old-emb'));
        if (id_fam === undefined || id_fam === 0 || isNaN(id_fam)) { alert("Identifaction de la famille impossible.\nCode erreur : LYARN5EO"); return false; }
...
function modalNouvelEmballageFrontEtape2Listener() {
    ...
    $('.carte-new-emb-defaut').off("click.cartenewembdefaut'").on("click.cartenewembdefaut", function(e) {
        ...
        $.fn.ajax({
            'rep_script_execute': "../scripts/ajax/",
            'script_execute': 'fct_emballages.php',
            'arguments': 'mode=setNewEmballageEnCours&code_vue=srgh&id_emb='+id_emb+'&id_froid='+id_froid,
            'done': function() {
...
function saveTemperature(id_froid) {
    ...
    $.fn.ajax({
        ...
        'callBack': function (retour) {
            ...
            if (parseInt(retour) < 0) { alert("Une erreur est survenue !\r\nEnregistrement impossible...\r\nCode erreur : SYQMYRGY"); return false; }


            // Pas de Loma ici, c'est déj fait, on saut l'étape 7 : Etape 8 SRGH
            chargeEtape(8,0);
            return false;
...
function saveProduit(identifiant, id_palette, id_compo) {
    ...
    if (modeUpd) {
        ...
        setTimeout(function(){
            ...
            $.fn.ajax({
                ...
                'done': function () {
                    $('.btnValiderCode i.spinner').removeClass('fa-spinner fa-spin').addClass('fa-check');
                    $('#inputIdFroid').val(id_froid);
                    chargeEtape(10,0);
    ...
    // Sinon, c'est qu'on est dans l'ajout d'un produit dans le traitement
        ...
        $.fn.ajax({
            ...
            'callBack': function (id_froid_retour) {

                id_froid_retour+= '';

                // Gestion des erreurs
                if (parseInt(id_froid_retour) < 0) {
                    alert("Une erreur est survenue !\r\nCode erreur : JXLHNWIQ-"+id_pdt+"-"+id_lot+"-"+id_froid+"-"+nbColis+"-"+poidsPdt+"-"+quant+"-"+id_palette);
                    return false;
                }

                // Intégration de l'id froid si on viens de la créer et que le retour est valide
                if (parseInt(id_froid_retour) > 0 && id_froid_retour !== undefined && !isNaN(id_froid_retour) && parseInt($('#inputIdFroid').val()) === 0) {
                    $('#inputIdFroid').val(id_froid_retour);
                    id_froid = parseInt(id_froid_retour);
                }
                ...
                modalLomaListener(id_pdt, id_lot, id_froid_retour);
...
function saveProduitAttente(identifiant, id_palette, id_compo) {
    ...
    if (isNaN(parseInt(identifiant)) || parseInt(identifiant) === 0 || parseInt(identifiant) === undefined ||
        ...
        alert("ERREUR !\r\nIdentification des données impossible...\r\nCode erreur : C0WINBY7-"+identifiant+"/"+id_palette+"/"+id_compo+"/"+nbColis+'/'+poidsPdt);
        return false;
    }
