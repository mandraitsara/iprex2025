...
/*
 ...
 _|        _|    _|  _|
   _|_|_|  _|_|_|    _|_|_|_|
--------------------------------------------------------
Contrôleur Ajax VUE SURGELATION HORIZONTALE
------------------------------------------------------*/
...
function modeChargeEtapeVue() {
	...
	if ($etape == 0) { ?>
        ...
        <div class="row mt-5">
            <div class="col text-center">
                <i class="fa fa-ruler-horizontal gris-9 fa-6x"></i>
            </div>
        </div>
        ...
        <div class="row mt-5">
            ...
            <div class="col-5 offset-1">
                <button type="button" class="btn btn-info btn-lg form-control btnNouvelleSrg">
                    ...
                    <h3 class="mb-3">Nouvelle surgélation horizontale&hellip;</h3>
                </button>
            </div>
            ...
            <div class="col-5">
                <button type="button" class="btn btn-secondary btn-lg form-control btnSrgsEnCours">
                    ...
                    <h3 class="mb-3">Surgélations horizontales en cours&hellip;</h3>
                </button>
            </div>
                    ...
                    <div class="card-body">
                        <table>
							...
							<?php if ($utilisateur->isDev()) { ?>
                                <tr>
                                    <td class="vmiddle"><i class="fa fa-user-secret"></i> id_lot_pdt_froid</td>
                                    <th><code class="text-warning"><?php echo $pdt_attente->getId_lot_pdt_froid(); ?></code></th>
                                </tr>
                                <tr>
                                    <td class="vmiddle"><i class="fa fa-user-secret"></i> id_palette</td>
                                    <th><code class="text-warning"><?php echo $pdt_attente->getId_palette(); ?></code></th>
                                </tr>
                                <tr>
                                    <td class="vmiddle"><i class="fa fa-user-secret"></i> id_compo</td>
                                    <th><code class="text-warning""><?php echo $pdt_attente->getId_compo() != '' ? $pdt_attente->getId_compo() : '-'; ?></code></th>
                                </tr>
							<?php } ?>
                        </table>
                    </div>
		...
		if (empty($listeLot)) {
            ...
            <div class="col alert alert-secondary mt-3 padding-50">
                <h2 class="mb-0 text-secondary text-center"><i class="fa fa-exclamation-circle fa-2x mb-3"></i>
                    <p>Aucune lot disponible&hellip;</p>
                </h2>
            </div>
	...
	if ($etape == 3) {
		...
		if (isset($couplePdtLotArray[2]))  {
			...
			if ($froidProduit instanceof FroidProduit) {
				...
				$id_lot_pdt_froid  = $froidProduit->getId_lot_pdt_froid();
            }
        ...
        <div class="row mt-2 align-content-center">
            ...
            <div class="col-8">
                  <div class="row">
                    <div class="col">
                        <h4><i class="fa fa-angle-down fa-lg ml-2 mr-3"></i><?php echo $pdt->getVrac() == 1 ? 'Poids' : 'Nombre de colis'; ?> en surgélation :</h4>
                    </div>
                    <div class="col text-right">
						...
						if ($id_froid > 0) {
							...
							if ($froidPdt instanceof FroidProduit) {
								...
								$quantiteOld = $froidPdt->getQuantite();
							}
            ...
            <div class="col-12 ">
				...
				if ($utilisateur->isDev()) {
                    ...
                    if ($restantPoids <= 0 || $restantColis <= 0) {
						...
						$log->setLog_texte("[SRGH] Mise à jour statut palette #".$palette->getId()." (capacité atteinte) durant affichage liste ") ;
	...
	if ($etape == 4) {
        ...
        <div class="row mt-5 align-content-center">
            ...
            <div class="col-7">
				...
				$srgh_consignes_debut = $configManager->getConfig('srgh_consignes_debut');
				if ($srgh_consignes_debut instanceof Config) {
					if (strlen(trim($srgh_consignes_debut->getValeur())) > 0) { ?>
                        ...
                        <div class="row">
                            <div class="col mt-3">
                                <div class="alert alert-secondary">
                                    ...
                                    <div>
										<?php echo $srgh_consignes_debut->getValeur(); ?>
                                    </div>
                                </div>
                            </div>
                        </div>
	...
	if ($etape == 5) {
        ...
        <div class="row mt-5 align-content-center">
            <div class="col text-center">
                <h4><i class="fa fa-angle-down fa-lg mr-3"></i>Entrée en surgélateur :</h4>
	...
	if ($etape == 6) {
		...
		if ($froid->getDate_sortie() == '' || $froid->getDate_sortie() == '0000-00-00 00:00:00' || $froid->getDate_sortie() == null) {
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Enregistrement fin de surgélation OP_froid #" . $id_froid) ;
			$logsManager->saveLog($log);
            ...
            if ($activationAlerte instanceof Config && intval($activationAlerte->getValeur()) == 1 && $froid->getNuit() == 0) {
           		...
           		if ($config_duree_srgh_max instanceof Config) {
                    ...
                    $dureeMax = floatval($config_duree_srgh_max->getValeur()); // En float pour comparaison avec la diff
                    if ($dureeMax > 0.0) {
                        ...
                        if ($diff > $dureeMax) {
                            ...
                            $id_froid_type = intval($froidManager->getFroidTypeByCode('srgh'));
	...
	/** --------------------------------------------------------
	 * Etape        : 7
	 * Description  : Contrôle LOMA (AVANT / PDT)
	 * Paramètres   : Froid
	 *  ----------------------------------------------------- */
	...
	if ($etape == 7) {
		...
		if (!$froidProduit instanceof FroidProduit) { ?>
            <div class="alert alert-danger padding-50 mt-5">
                <h4>ERREUR !</h4>
                <p>Identification du produit/froid impossible...</p>
                <code>Code erreur : 5RDCDJP1</code>
            </div>
			<?php exit;
		}
        ...
        <form id="controleLoma">
            ...
            <div class="row">
                <div class="col text-center header-loma">
                    ...
                    <div class="badge badge-dark text-24 badge-pill mt-3 <?php echo $avantAfaire ? 'hid' : ''; ?>"><?php echo $froidProduit->getProduit()->getNom(); ?></div>
                    <div class="badge badge-dark text-24 badge-pill mt-3 <?php echo $avantAfaire ? '' : 'hid'; ?>">Passage des tests avant produits</div>
                </div>
            </div>
	...
	/** --------------------------------------------------------
	 * Etape        : 12
	 * Description  : Contrôle LOMA (APRES)
	 * Paramètres   : Froid
	 *  ----------------------------------------------------- */
	...
	if ($etape == 8) { ?>
        ...
        <div class="row mt-3 align-content-center">
            <div class="col text-center">
                ...
                <div class="alert alert-secondary">
                    <h4><i class="fa fa-angle-down fa-lg ml-2 mr-3 mb-3"></i>Emballages disponibles en Surgélation Horizontale  :</h4>
	...
	if ($etape == 11) {
        ...
        <div class="col-8">
            <div class="row">
                <div class="col">
                    <h4><i class="fa fa-angle-down fa-lg ml-2 mr-3"></i><?php echo $froidProduit->getProduit()->getVrac() == 1 ? 'Poids' : 'Nombre de colis'; ?> en surgélation :</h4>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <!-- Nombre de colis -->
                <div class="col-5 <?php echo $froidProduit->getProduit()->getVrac() == 1 ? 'd-none' : '';?>">
                    <div class="input-group">
	...
	if ($etape == 101) {
		...
		if (empty($listeSrgEnCours)) {
			...
			if ($utilisateur->isDev()) {
				$_SESSION['infodevvue'] = '<i class="fa fa-ban text-danger"></i> Aucun objet trouvé [type SRGH] [statut 0|1] ';
			}
        ...
        <div class="row">
            <div class="col mt-3">
                <h4><i class="fa fa-angle-down fa-lg ml-2 mr-3"></i>Surgélations horizontales en cours :</h4>
            </div>
        </div>
		...
		if ($utilisateur->isDev()) {
			...
			$_SESSION['infodevvue'] = count($listeSrgEnCours) . ' objet'.$s.' trouvé'.$s.' [type SRGH] [statut 0|1] ';
		}
		...
		foreach ($listeSrgEnCours as $froid) {
            ...
            <div class="card text-white bg-info mb-3 carte-srg d-inline-block mr-3" style="max-width: 20rem;" data-id-froid="<?php echo $froid->getId(); ?>">
                ...
                <div class="card-header text-36"><?php echo 'SRGH'.sprintf("%04d", $froid->getId()); ?></div>
							...
							if ($froid->isSortie()) { ?>
                                ...
                                <th class="text-right text-16"><?php
                                    ...
                                    <?php } else {
										...
										$sortieEstime = date('Y-m-d H:i',strtotime('+6 hour',strtotime($froid->getDate_entree())));
...
function modeChargeTicketLot() {
	 ...
	 *                avant le numéro de SRGH dans le ticket
	 *  ------------------------------------------------- */
	...
	if ($id_froid > 0) {
		...
		if ($froid instanceof Froid) {
            ...
            <div class="alert bg-info text-24 text-center mb-1 <?php echo $etape < 4 ? 'mt-4' : ''; ?>">
                <i class="fa fa-ruler-horizontal text-20"></i>
				<?php echo 'SRGH'.sprintf("%04d", $froid->getId()); ?>
            </div>
...
function modeShowProduitsFamille($id_famille = 0, $id_lot = 0, $exit = true) {
	...
	$froidManager = new FroidManager($cnx);
	$froidType = $froidManager->getFroidTypeByCode('srgh');
                ...
                <div class="row">
					...
					if (empty($liste_produits)) { ?>
                        ...
                        <div class="col-12 padding-50">
                            ...
                            <p><i>Famille <?php echo $famille->getNom();?> en surgélation horizontale</i></p>
...
MODE -  Fonction déportée pour mise à jour
	du détail stock emballage de la SRGH
----------------------------------------------------------*/
...
function setEmballageProd($id_froid) {
	...
	if (empty($emballagesFroid)) {
		...
		$froidManager = new FroidManager($cnx);
		$vueSrg = $vueManager->getVueByCode('srgh');
		if (!$vueSrg instanceof Vue) { ?><div class="alert alert-danger">Identification de la vue impossible.<br>Code Erreur : <code>QJXJTZDR</code></div><?php  exit; }

		$froid = $froidManager->getFroid($id_froid);
		if (!$froid instanceof Froid) { ?><div class="alert alert-danger">Identification du traitement impossible.<br>Code Erreur : <code>UJCFPIYN</code></div><?php  exit; }
		...
		if ($consommablesManager->setEmballagesVue($vueSrg, $froid)) {
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Association des emballages au traitement " . $id_froid) ;
			$logsManager->saveLog($log);
		}
...
function modeAddPdtFroid($params = []) {
	...
	if (!$params || empty($params)) {
		...
		$quantite          = isset($_REQUEST['quantite'])         ? intval($_REQUEST['quantite'])      : 0;
		$nb_colis_add   = isset($params['nb_colis_add'])  ? intval($params['nb_colis_add']) : $nb_colis;
		$poids          = isset($params['poids'])           ? floatval($params['poids'])        : 0.0;
		$poids_add      = isset($params['poids_add'])     ? floatval($params['poids_add'])  :$poids;
	...
	if ($id_froid == 0) {
		...
		$typeCgl = $froidManager->getFroidTypeByCode('srgh');
		...
		// Puisqu'on viens de créer l'op de froid, le lot concerné est officiellement dans la vue surgélation horizontale
		$lotsManager->addLotVue($lot, 'srgh');       // Affecte le lot à la vue surgélation horizontale

		// Log
		$log = new Log([]);
		$log->setLog_type('info');
		$log->setLog_texte("[SRGH] Création du traitement OP froid ID " . $id_froid . " à l'ajout du premier produit.") ;
		$logsManager->saveLog($log);
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] ".$logVerbe." du produit ID " . $id_pdt . " sur le lot ID " . $id_lot . " à l'OP froid ID " . $id_froid . " quantième " . $quantieme . ", nb_colis = ". $nb_colis." poids = " . $poids) ;
	$logsManager->saveLog($log);
...
function modeControleLomaPdt() {
	...
	$id_lot     = isset($_REQUEST['id_lot'])    ? intval($_REQUEST['id_lot'])   : 0;
	$id_froid   = isset($_REQUEST['id_froid'])  ? intval($_REQUEST['id_froid']) : 0;
	if ($id_pdt == 0 || $id_lot == 0 || $id_froid == 0) { echo '-2'; exit; }
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Définition du contrôle LOMA requis sur le produit ID_LOT_PDT_FROID " . $froidProduit->getId_lot_pdt_froid()) ;
	$logsManager->saveLog($log);
...
function modeListeCartesEmballage() {
	...
	$params = [
		'id_vue'            => $vuesManager->getVueByCode('srgh')->getId(),
		'get_emb'           => true,
		'has_encours'       => true,
		'start'             => $start,
		'nb_result_page'    => $nbResultPpage
...
function modeUpdPdtLotPoids() {
	...
	if ($nb_colis == 0) {
		...
		$log = new Log([]);
		$log->setLog_type('info');
		$log->setLog_texte("[SRGH] Suppression du produit ID_LOT_PDT_FROID " . $id_lot_pdt_froid . " du traitement OP Froid " . $froidProduit->getId_froid()) ;
		$logsManager->saveLog($log);
	...
	if ($ajout != 1 && $froidProduit->getId_palette() == $id_palette) {
        ...
        if ($id_last > 0) {
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] suppression des compos en trop (modif poids en mode total) pour l'ID_LOT_PDT_FROID " . $id_lot_pdt_froid) ;
			$logsManager->saveLog($log);
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Modification du produit depuis récap traitement, ID_LOT_PDT_FROID " . $id_lot_pdt_froid) ;
	$logsManager->saveLog($log);
...
function modeEtiquetageProduitLot() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] ".$logVerbe." produit ID_LOT_PDT_FROID " . $id_lot_pdt_froid) ;
	$logsManager->saveLog($log);
...
function modeSaveTempDebut() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Saisie de la température en début de traitement à " . $temp_debut . "°C pour l'OP Froid ID " . $id_froid) ;
	$logsManager->saveLog($log);
...
function modeSaveTempFin() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Saisie de la température de fin de traitement à " . $temp_fin . "°C pour l'OP Froid ID " . $id_froid) ;
	$logsManager->saveLog($log);
...
function modeSaveDebutSrg() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Début du traitement ".$texteNuit." de l'OP Froid ID " . $id_froid) ;
	$logsManager->saveLog($log);
...
function modeConformiteSrg() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] ".$logConformite." du traitement OP froid produit " . $id_froid) ;
	$logsManager->saveLog($log);
...
function modeClotureSrg() {
	...
	$vuesManager = new VueManager($cnx);
	$vueRcp = $vuesManager->getVueByCode('srgh');
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Clôture du traitement " . $id_froid) ;
	$logsManager->saveLog($log);
...
function modeSaveLoma() {
	...
	// Récupération des variables
	$id_lot_pdt_froid   = isset($_REQUEST['id_lot_pdt_froid'])  ? intval($_REQUEST['id_lot_pdt_froid']) : 0;
	if ($id_lot_pdt_froid == 0) { exit('250INXYW'); }
	...
	if ($resultest_nfe >= 0) {
		...
		if ($froid->getTest_avant_fe() < 0) {
		    ...
		    if (!$froidManager->saveFroid($froid)) { exit('ERR_SAVEFROID_TESTSAVANT_'.$id_froid);}

			// Log
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Enregistrement contrôle LOMA AVANT sur froid #".$id_froid) ;
			$logsManager->saveLog($log);
	...
	if ($ressave) {
		...
		$log = new Log([]);
		$log->setLog_type('info');
		$log->setLog_texte("[SRGH] Contrôle LOMA sur produit ID_LOT_PDT_FROID " . $id_lot_pdt_froid) ;
		$logsManager->saveLog($log);
		...
		$vue = $vueManager->getVueByCode('srgh');
		if (!$vue instanceof Vue) { exit('3V1GV2E8'); }
		if (intval($loma->getId()) == 0) { exit('MYWYY8QB'); }

		// On récupère le lot depuis le lotpdtfroid
		$lotpdtFroid = $froidManager->getFroidProduitObjetByIdLotPdtFroid($id_lot_pdt_froid);
		if (!$lotpdtFroid instanceof FroidProduit) { exit('NONMNT2Y'); }
...
function modeSaveCommentaireTempHs() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Enregistrement commentaire Temps HS sur lot #".$id_lot." froid #" . $id_froid) ;
	$logsManager->saveLog($log);
...
function modeReprisePdtAttente() {
	...
	if ($id_lot_pdt_froid == 0 || $id_palette == 0 || $id_compo == 0 || $nb_colis == 0 || $poids == 0.0) { exit('O67XWBZF'); }
	...
	if (!$pdtFroid instanceof FroidProduit) { exit('AMKR6Q5V'); }

	// Si l'opération de froid n'existe pas encore, on la crée...
	if ($id_froid == 0) {

		$typeSrgh = $froidManager->getFroidTypeByCode('srgh');
		if (!$typeSrgh || $typeSrgh == 0) { exit('0XQZFTRB'); }

		$froid = new Froid([]);
		$froid->setId_type($typeSrgh);
		$froid->setId_user_maj($utilisateur->getId());
		$id_froid = $froidManager->saveFroid($froid);
		if (!$id_froid || $id_froid == 0) { exit('ETYMOSLG'); }
		...
		if (!$lot instanceof Lot) { exit('OBXBWEDC'); }
	...
	if ($poids == $pdtFroid->getPoids() && $nb_colis == $pdtFroid->getNb_colis()) {
		...
		if ($pdtFroidAcompleter instanceof FroidProduit) {
			...
			if (!$froidManager->saveFroidProduit($pdtFroidAcompleter)) { exit('URX0NX94'); }

			// Il n'y donc plus rien dans le produit en attente, on le supprime
			if (!$froidManager->supprPdtfroid($pdtFroid)) { exit('RPFHHKNQ'); }
			...
			if (intval($id_client) == 0) {
				...
				if (intval($id_client) == 0) { exit('YBBXLNER'); }
			}
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Création d'une compo cumulée sur pdt_froid" . $pdtFroidAcompleter->getId_lot_pdt_froid()) ;
			$logsManager->saveLog($log);
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Retrait du statut attente et affectation au nouveau traitement sur pdt_froid #" . $pdtFroid->getId_lot_pdt_froid()) ;
			$logsManager->saveLog($log);
		...
		if ($pdtFroidAcompleter instanceof FroidProduit) {
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Cumul des quantités sur produit en attente #" . $pdtFroidAcompleter->getId_lot_pdt_froid()." depuis #".$pdtFroid->getId_lot_pdt_froid()) ;
			$logsManager->saveLog($log);
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Création d'un nouveau produit froid #".$new_id_lot_pdt_froid." avec les quantités choisies pour reprise pdt en attente #" . $pdtFroid->getId_lot_pdt_froid()) ;
			$logsManager->saveLog($log);
		...
		if (!$froidManager->saveFroidProduit($pdtFroid)) { exit('8G67J0EK'); }

		// Log
		$log = new Log([]);
		$log->setLog_type('info');
		$log->setLog_texte("[SRGH] Retrait des quantités du produit froid #".$pdtFroid->getId_lot_pdt_froid()." initial") ;
		$logsManager->saveLog($log);
		...
		if ($pdtFroid->getId_compo() > 0) {
			...
			if ($oldCompo instanceof PaletteComposition) {
				...
				if (!$palettesManager->savePaletteComposition($oldCompo)) { exit('OYXCFQ82'); }

				// Log
				$log = new Log([]);
				$log->setLog_type('info');
				$log->setLog_texte("[SRGH] Mise à jour des quantités de la compo #".$oldCompo->getId()." pour produit_froid #".$pdtFroid->getId_lot_pdt_froid()) ;
				$logsManager->saveLog($log);
				...
				$log = new Log([]);
				$log->setLog_type('info');
				$log->setLog_texte("[SRGH] Suppression des autres compos liées pdt_froid #".$id_lot_pdt_froid." excéptée la compo #". $pdtFroid->getId_compo()) ;
				$logsManager->saveLog($log);
				...
				$id_new_compo = $palettesManager->savePaletteComposition($newCompo);

				if (!$id_new_compo) { exit('97WOU9Q0'); }

				// Log
				$log = new Log([]);
				$log->setLog_type('info');
				$log->setLog_texte("[SRGH] Création de la compo #".$id_new_compo." du nouveau produit splité sur produit_froid #" . $new_id_lot_pdt_froid) ;
				$logsManager->saveLog($log);
	...
	if ($compo instanceof PaletteComposition) {
		...
		if (isset($new_id_lot_pdt_froid)) {
			...
			$log = new Log([]);
			$log->setLog_type('info');
			$log->setLog_texte("[SRGH] Mise à jour de l'id_lot_pdt_froid #".$new_id_lot_pdt_froid." dans la compo #".$id_compo." apres création d'un nouveau dans reprise partielle.") ;
			$logsManager->saveLog($log);
		}
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Purge des compos temporaires et supprimées sur id_lot_pdt_froid #".$id_lot_pdt_froid) ;
	$logsManager->saveLog($log);
...
function modeSaveLomaApres() {
	...
	$log = new Log([]);
	$log->setLog_type('info');
	$log->setLog_texte("[SRGH] Enregistrement contrôle LOMA APRES sur froid #".$id_froid) ;
	$logsManager->saveLog($log);
