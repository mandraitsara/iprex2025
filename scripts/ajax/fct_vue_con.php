...
/*
 ...
 _|        _|    _|  _|
   _|_|_|  _|_|_|    _|_|_|_|
--------------------------------------------------------
Contrôleur Ajax VUE CONSOMMABLES
------------------------------------------------------*/
        ...
        <div class="row mt-3 align-content-center">
            <div class="col text-center">
                <div class="alert alert-secondary">
                    <h4><i class="fa fa-angle-down fa-lg ml-2 mr-3"></i>Consommables disponibles :</h4>
                    <div class="row" id="containerListeConsommables">
						<?php
						modeListeCartesConsommables();
						?>
                    </div>
                </div>
            </div>
...
function modeListeCartesConsommables() {
	...
	$filtresPagination  = '?mode=listeCartesConsommables';
	$start              = ($page-1) * $nbResultPpage;

	$params = [
		'get_emb'           => true,
        'not_id_type'       => 1, // On exclu le type 1 (emballages) pour avoir tous les autres
		'has_encours'       => true,
        'get_nom_type'      => true,
		'start'             => $start,
	...
	foreach ($famillesListe as $fam) { ?>
        ...
        <div class="col-2 mb-3">
            <div class="card bg-secondary text-white carte-emb" data-id-fam="<?php
                ...
                <div class="card-header gris-c texte-fin padding-5">
					<?php echo $fam->getNom_type();?>
                </div>

                <div class="card-body">

                    <h5 class="card-title mb-0 <?php echo strlen($fam->getNom()) > 30 ? 'text-16' : 'text-18'; ?>"><?php echo strtoupper($fam->getNom()); ?></h5>

                    <div class="badge badge-dark text-16 d-block margin-top-15 texte-fin">
						<span class="texte-fin"><?php echo $fam->getEmb_encours() instanceof Consommable ? $fam->getEmb_encours()->getNumlot_frs() : '?'; ?></span>
                        <hr class="mb-1 mt-1">

                        <span class="text-14 margin-top-5 texte-fin d-block">DLC : <?php if ($fam->getEmb_encours() instanceof Consommable) {
								echo $fam->getEmb_encours()->getDate_dlc() != '' && $fam->getEmb_encours()->getDate_dlc() != '0000-00-00' ? Outils::dateSqlToFr($fam->getEmb_encours()->getDate_dlc()) : 'N/A';
							}  ?></span>


                        <span class="text-12 margin-top-10 texte-fin d-block"><?php echo $fam->getEmb_encours() instanceof Consommable ? Outils::cleanCut($fam->getEmb_encours()->getNom_frs(),30) : ''; ?></span>
                    ...
                    <div class="row">
                        <div class="col">
                            <button type="button" class="btn btn-danger padding-20-10 border-light form-control btn-con-defectueux" data-id-con="<?php
                        ...
                        <div class="col mt-2">
                            <button type="button" class="btn btn-info padding-20 border-light form-control btn-emb-change" data-id-old-con="<?php echo $fam->getEmb_encours() instanceof Consommable ? $fam->getEmb_encours()->getId() : ''; ?>"><i class="fa fa-retweet fa-lg"></i></button>
                        </div>
	...
	if (isset($pagination)) {
		...
		$pagination->setNature_resultats('consommable');
...
function modeChargeTicketLot() {
	...
	?>
        <h4 class="mb-3">Consommables défectueux</h4>
        <table id="ticketLotInfoStock" class="table-suivi-stock-emb"><?php showConsommableProd(); ?></table>
        <h4 class="mb-3 mt-3">Changements de références</h4>
        <table id="ticketLotInfoStockD" class="table-suivi-stock-emb"><?php showConsommableChang(); ?></table>
		<?php
...
function showConsommableProd() {
	...
	$defectueux = $consommablesManager->getConsommablesDefectueuxJour();
...
function showConsommableChang() {

	global $cnx, $consommablesManager;

	// On récupère les changements de rouleaux du jour pour ce lot
	$changements = $consommablesManager->getConsommablesChangementReferenceJour();
...
// Fonction déporté pour affichage dynamique du stock du jour des autres consommables
function  showConsommablesStockJour() {

    global $cnx;
	$consommablesManager = new ConsommablesManager($cnx);

    // On récupère la liste
    $liste = $consommablesManager->getStocksJourConsommables();
    if (!empty($liste)) { */?><!--

        <h4 class="mb-3">Utilisation du jour :</h4>
        <table class="table-suivi-stock-emb">
            <?php
/*            foreach ($liste as $famille => $totalJour) { */?>
                <tr>
                    <td class="text-14"><i class="fa fa-shopping-cart mr-2 fa-fw gris-c"></i><?php /*echo $famille; */?></td>
                    <td class="text-right hook-badge-emb-ticket">
                        <span class="badge badge-info badge-pill text-16"><?php /*echo $totalJour; */?></span>
                    </td>
                </tr>
            <?php /*} // FIN boucle familles mouvements
            */?>
        </table>
