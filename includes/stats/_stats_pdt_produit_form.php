<form action="<?php echo basename($_SERVER['PHP_SELF']);?>" method="post" id="formPdtsClts">
	<input type="hidden" name="action" value="pdt"/>
	<input type="hidden" name="onglet" value="pdtsClts"/>
	...
	<div class="row">
		...
		<div class="col-3">
			<div class="input-group">
				...
				<select class="selectpicker show-tick form-control" name="id_produit" title="Sélectionnez" data-live-search="true" data-live-search-placeholder="Rechercher" data-size="12">
					<?php
					foreach($produitsManager->getListeProduits() as $pdtselect) { ?>
						<option value="<?php echo $pdtselect->getId(); ?>" data-subtext="<?php echo $pdtselect->getCode(); ?>" <?php echo $id_produit == $pdtselect->getId() ? 'selected' : ''; ?>><?php
