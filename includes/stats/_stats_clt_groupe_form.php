<form action="<?php echo basename($_SERVER['PHP_SELF']);?>" method="post" id="formCltGrp">
	<input type="hidden" name="action" value="clt"/>
	<input type="hidden" name="onglet" value="cltGrp"/>
		...
		<div class="col-3">
			<div class="input-group">
				...
				<select class="selectpicker show-tick form-control" name="id_groupe" title="Sélectionnez">
					<?php
					foreach($tiersManager->getListeTiersGroupes() as $grpSelect) { ?>
						<option value="<?php echo $grpSelect->getId(); ?>" <?php echo $id_groupe == $grpSelect->getId() ? 'selected' : ''; ?>><?php
							echo $grpSelect->getNom(); ?></option>
					<?php }
					?>
				</select>
			</div>
