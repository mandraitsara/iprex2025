<?php if ($show_marges && !empty($liste)) {
    ...
    <table class="table admin table-v-middle table-marges">
        ...
        <tr>
            <td>Taux de marge brute</div></td>
          
            <th class="text-right w-court-admin-cell texte-fin bg-secondary"><?= ($total_ca > 0) ? number_format(($total_marge_brute * 100) / $total_ca,$deci,'.', ' ') : number_format(($total_marge_brute * 100),$deci,'.', ' ');?> %</th>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <th class="text-right w-court-admin-cell texte-fin bg-secondary"><?= ($total_ca > 0) ? number_format((($total_marge_brute-$ff) * 100) / $total_ca,$deci,'.', ' ') : number_format((($total_marge_brute-$ff) * 100),$deci,'.', ' ');?> %</th>
        </tr>
    </table>
    <?php
